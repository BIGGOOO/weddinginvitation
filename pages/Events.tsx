...
const Events: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-[#FCFBF4] py-16 sm:py-24 md:py-32 px-4 relative overflow-x-hidden">
      <div className="absolute top-0 left-0 w-full h-full pointer-events-none opacity-25">
        <div className="absolute top-1/4 -right-20 w-[300px] sm:w-[600px] h-[300px] sm:h-[600px] bg-gold/5 rounded-full blur-[100px]" />
        <div className="absolute bottom-1/4 -left-20 w-[300px] sm:w-[600px] h-[300px] sm:h-[600px] bg-maroon/5 rounded-full blur-[100px]" />
      </div>

      <div className="max-w-6xl mx-auto relative z-10">
        <header className="text-center mb-16 sm:mb-24 md:mb-40 reveal px-4">
          <div className="mb-4 sm:mb-6 inline-block px-4 py-2 bg-maroon/5 border border-maroon/10 rounded-full">
            <span className="text-[9px] sm:text-[10px] font-bold text-maroon uppercase tracking-[0.3em] sm:tracking-[0.4em]">Protocol Timeline</span>
          </div>
          <h1 className="text-5xl sm:text-7xl md:text-9xl text-maroon mb-6 sm:mb-8 leading-tight serif">The Timeline</h1>
        ...
        <div ref={timelineRef} className="relative pb-24 sm:pb-40 px-2 sm:px-0">
          ...
          <div className="space-y-20 sm:space-y-32 md:space-y-20 relative">
                ...
                >
                  <div 
                    className={`absolute left-0 md:left-1/2 md:-translate-x-1/2 top-1/2 -translate-y-1/2 w-8 h-8 sm:w-10 sm:h-10 rounded-full border-4 border-[#FCFBF4] z-30 transition-all duration-700 ${
                      isActive ? 'bg-maroon scale-110 sm:scale-125 shadow-xl' : 'bg-slate-300'
                    }`}
                  >
                    {isActive && (
                  ...
                  <div className={`w-full md:w-1/2 pl-10 sm:pl-12 md:pl-0 ${isEven ? 'md:order-last md:pl-24' : 'md:pr-24 md:text-right'}`}>
                    <div className={`bg-white p-6 sm:p-8 md:p-12 rounded-[2.5rem] sm:rounded-[3.5rem] shadow-md border transition-all duration-1000 ease-out ${
                      isActive 
                        ? 'border-gold shadow-2xl translate-y-0 opacity-100 scale-[1.01] sm:scale-[1.02]' 
                        : 'border-slate-100 translate-y-8 sm:translate-y-12 opacity-0'
                    }`}>
                      <div className="flex flex-col gap-4 sm:gap-6">
                        <div className={`flex items-start gap-4 ${!isEven ? 'md:flex-row-reverse' : ''}`}>
                          ...
                          <div className="flex-1">
                            <div className="text-[8px] sm:text-[10px] font-bold text-maroon tracking-[0.2em] sm:tracking-[0.3em] uppercase mb-1">Day {event.day} • {event.date}</div>
                            <h3 className="serif text-2xl sm:text-3xl md:text-4xl text-slate-800 leading-tight">{event.title}</h3>
                          </div>
                        </div>

                        <div className={`flex flex-wrap gap-2 sm:gap-3 text-slate-500 font-bold text-[9px] sm:text-xs ${!isEven ? 'md:justify-end' : ''}`}>
                          <span className="flex items-center gap-2 px-3 sm:px-4 py-2 bg-slate-50 rounded-full border border-slate-100 whitespace-nowrap">🕒 {event.time}</span>
                          <span className="flex items-center gap-2 px-3 sm:px-4 py-2 bg-slate-50 rounded-full border border-slate-100">📍 {event.location}</span>
                        </div>

                        {event.note && (
                          <div className={`p-3 sm:p-4 border-l-2 bg-slate-50 rounded-r-2xl transition-all ${isActive ? 'border-gold' : 'border-slate-200'}`}>
                            <p className={`text-[11px] sm:text-[12px] italic leading-relaxed text-slate-500 ${!isEven ? 'md:text-right' : ''}`}>
                              “{event.note}”
                            </p>
                          </div>
                        )}

                        <div className={`flex gap-2 sm:gap-3 pt-2 sm:pt-4 ${!isEven ? 'md:justify-end' : ''}`}>
                          <button 
        ...
        <section className="pt-24 sm:pt-32 border-t border-slate-200 reveal px-4">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 sm:gap-12">
            {[
              ...
              <div key={idx} className="bg-white p-6 sm:p-10 rounded-[2.5rem] sm:rounded-[3rem] shadow-sm border border-slate-100 hover:shadow-2xl transition-all group overflow-hidden relative">
                ...
                <button 
                  ...
                  className="w-full py-4 sm:py-5 bg-slate-50 hover:bg-slate-900 hover:text-white rounded-full text-[9px] sm:text-[10px] font-bold text-maroon transition-all uppercase tracking-[0.2em] sm:tracking-[0.3em] flex items-center justify-center gap-3"
                >
                  START NAVIGATION ↗
                </button>
              </div>
